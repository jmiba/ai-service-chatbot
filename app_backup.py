...
def render_with_citations_by_index(text_html, citation_map, placements):
    ...
    Use a simpler approach that preserves markdown formatting.
    """
    s = text_html or ""
    n = len(s)
    ...
    for idx, num in sorted(placements, key=lambda x: x[0], reverse=True):
        ...
        # Use markdown-compatible citations that don't interfere with formatting
        citation = f"[{num}]"
        i = max(0, min(n, idx))
        s = s[:i] + citation + s[i:]
...
def handle_stream_and_render(user_input, system_instructions, client, retrieval_filters=None, debug_one=False):
    ...
    with st.chat_message("assistant"):
        ...
        # First clean malformed citation data from raw response
...
# Temporary debug - show this info at the bottom of the page
st.markdown("---")
st.markdown(f"**Debug:** Last error code: {last_error_code}, Show button: {show_contact_button}")
